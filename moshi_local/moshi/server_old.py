...
class ContextCompressor:
    ...
    def compress(self, question: str, history: list[tuple[str, str]], chunks: list[dict]) -> str:
        if not chunks:
            return ""
        history_text = "\n".join(f"User: {u}\nMoshi: {m}" for u, m in history[-1:]) if history else "None"
        passages = "\n".join(
    ...
    async def handle_chat(self, request):
        ...
        async def start_rag_turn(transcript: str):
            ...
            def _quick_score():
                ...
                scores, _ = self.rag_index.search(q.reshape(1, -1), 1)
                return float(scores[0][0]) if scores.size else 0.0
        
            top_score = await loop.run_in_executor(None, _quick_score)
        
            if top_score < self.rag_trigger_score:
        ...
        async def _prepare_ref_block(transcript: str, loop: asyncio.AbstractEventLoop, my_epoch: int):
            ...
            if self.web_search_enabled and top_score < self.web_search_trigger_below:
                ...
                if web_hits:
                    ...
                    hits = hits[: self.rag_top_k + self.web_search_max_results]
                ...
                while True:
                    ...
                    while all_pcm_data.shape[-1] >= self.frame_size:
                        ...
                        if listening:
                            ...
                            if tokens is not None:
                                if awaiting_ref:
                                    # Model hasn't been grounded yet — don't let it speak an
                                    # ungrounded guess or "umm" filler. Keep stepping so its
                                    # cache stays in sync with the audio clock, but emit silence.
