...
into the text codebook. No vocabulary surgery, no new embedding row — the
model learns the convention purely from training exposure, same as it
already learned <system>.

Current turn shape (no lead/filler — the model never speaks between the
user's turn and the grounded answer):

    USER audio -> <lookup> -> <ref> reference text <ref> -> MODEL body

`<lookup>` is spliced at the `lookup` marker span's position (placed by data
prep immediately after the user's turn ends), and `<ref>` is spliced
immediately after the `<lookup>` block — see `_inject_rag_blocks`. Only
`lookup` + `body` spans are required; `lead`/`filler` spans are deprecated
(still parsed for backward compatibility with older sidecars, but ignored
by injection — see TurnSpan).
...
2. `InterleavedTokenizer.__call__` reads segment_meta + reference text from
   the sidecar JSON and calls `_inject_rag_blocks` to splice <lookup> then
   <ref> back-to-back right after the user's turn, before `body`.
...
@dataclass
class TurnSpan:
    kind: str          # "regular" | "lookup" | "ref" | "body"
                        # ("lead" / "filler" are deprecated: the old format let
                        # the model speak between the user's turn and <ref>;
                        # the current format never does -- see _inject_rag_blocks.
                        # Still parsed for backward compatibility with old data,
                        # but no longer required or produced by current data prep.)
    start_frame: int
    end_frame: int
...
class InterleavedTokenizer:
    ...
    def _inject_rag_blocks(
        self,
        text_tokens: torch.Tensor,
        audio_tokens: torch.Tensor,
        segment_meta: SegmentMeta,
        reference_text: str,
        this_num_audio_frames: int,
        path: str = "",
        start_sec: float = 0.0,
    ) -> tuple[torch.Tensor, torch.Tensor, SegmentMeta, bool, bool]:
        """Splices <lookup> then <ref> back-to-back at the `lookup` marker's
        position (placed by data prep immediately after the user's turn
        ends). No `filler` segment is required or used: the model never
        speaks between the user's audio and the grounded `body` answer --
        USER -> <lookup> -> <ref>...reference text...<ref> -> MODEL body.

        Only requires `lookup` + `body` spans (unlike the old lead/filler/ref
        pipeline, which also required `filler`). Returns (text_tokens,
        audio_tokens, segment_meta, injected_lookup, injected_ref)."""
        lookup_spans = segment_meta.spans_of("lookup")
        body_spans = segment_meta.spans_of("body")
        injected_lookup = injected_ref = False

        if not (lookup_spans and body_spans):
            if reference_text and _diag.hit("reference_no_spans"):
                logger.warning(
                    f"[diag] {path}@{start_sec:.1f}s: has reference_text but missing "
                    f"one of lookup/body spans — injection SKIPPED"
                )
            return text_tokens, audio_tokens, segment_meta, injected_lookup, injected_ref

        lookup_span = lookup_spans[0]
        if lookup_span.start_frame >= this_num_audio_frames:
            if _diag.hit("lookup_past_chunk_end"):
                logger.warning(
                    f"[diag] {path}@{start_sec:.1f}s: lookup_span.start_frame "
                    f"({lookup_span.start_frame}) >= chunk frames "
                    f"({this_num_audio_frames}) — injection SKIPPED, chunk boundary "
                    f"likely split the RAG turn"
                )
            return text_tokens, audio_tokens, segment_meta, injected_lookup, injected_ref

        n_q = audio_tokens.shape[1]

        lookup_text, lookup_audio = self._build_forced_text_frames(wrap_with_lookup_tags(), n_q=n_q)
        n_lookup = lookup_text.shape[-1]
        lookup_insert_at = lookup_span.start_frame
        text_tokens = torch.cat([text_tokens[..., :lookup_insert_at], lookup_text, text_tokens[..., lookup_insert_at:]], dim=-1)
        audio_tokens = torch.cat([audio_tokens[..., :lookup_insert_at], lookup_audio, audio_tokens[..., lookup_insert_at:]], dim=-1)
        segment_meta = self._shift_spans_from(segment_meta, lookup_insert_at, n_lookup)
        segment_meta.n_lookup_tokens = n_lookup
        injected_lookup = True

        # <ref> goes immediately after the <lookup> block. There is no filler
        # segment to skip over anymore -- the model has not spoken yet, so
        # the very next real audio/text after <ref> is the grounded `body`.
        # ref_insert_at is an absolute position in the (already-grown) tensor,
        # fixed independently of anything _shift_spans_from does below.
        ref_text, ref_audio = self._build_ref_block_frames(reference_text, n_q=n_q)
        n_ref = ref_text.shape[-1]
        ref_insert_at = lookup_insert_at + n_lookup
        text_tokens = torch.cat([text_tokens[..., :ref_insert_at], ref_text, text_tokens[..., ref_insert_at:]], dim=-1)
        audio_tokens = torch.cat([audio_tokens[..., :ref_insert_at], ref_audio, audio_tokens[..., ref_insert_at:]], dim=-1)
        segment_meta = self._shift_spans_from(segment_meta, ref_insert_at, n_ref)
        segment_meta.n_ref_tokens = n_ref
        injected_ref = True

        # Finalize "lookup" and "ref" spans to their exact injected extents,
        # only now that both splices are done. Doing this earlier (e.g.
        # re-pointing "lookup" right after its own splice) is wrong: the
        # generic shift-by-insertion-point applied for the <ref> splice would
        # then drag the lookup span's end boundary forward too, since it
        # sits exactly at the <ref> insertion point and "end_frame >=
        # insert_at" reads as "this span straddles the insertion." Data prep
        # typically supplies a zero-width lookup marker (start_sec ==
        # end_sec, just an insertion pointer) and never supplies a "ref" span
        # at all (there's no real audio/alignments to annotate it with) --
        # left uncorrected, make_segment_weight_tensor's `if s >= e: continue`
        # silently drops zero-width spans and --lookup-weight/--ref-context-
        # weight have no effect. Overwriting both here makes the injected
        # extents authoritative instead of trusting data prep to guess them.
        segment_meta.turns = [t for t in segment_meta.turns if t.kind not in ("lookup", "ref")]
        segment_meta.turns.append(TurnSpan(kind="lookup", start_frame=lookup_insert_at, end_frame=lookup_insert_at + n_lookup))
        segment_meta.turns.append(TurnSpan(kind="ref", start_frame=ref_insert_at, end_frame=ref_insert_at + n_ref))
    ...
    def _parse_segment_meta(self, raw_meta: dict, chunk_start_sec: float) -> SegmentMeta | None:
        ...
        # depends on it. ("lead"/"filler" are deprecated -- see TurnSpan --
        # but kept here so old-format sidecars still parse without warnings.)
        ...
        deprecated_kinds = sorted({s.kind for s in spans if s.kind in ("lead", "filler")})
        if deprecated_kinds and _diag.hit("deprecated_lead_filler_spans"):
            logger.warning(
                f"segment_meta contains deprecated turn kind(s) {deprecated_kinds} -- "
                f"the current data format does not use lead/filler (the model should not "
                f"speak before <ref>). These spans are parsed but ignored by RAG injection; "
                f"regenerate this sample with the current conversion scripts to remove them."
            )
    ...
    def __call__(self, wav: np.ndarray, start_sec: float, path: str) -> Sample:
        with torch.no_grad():
            ...
            has_rag_spans = segment_meta is not None and segment_meta.spans_of("lookup") and segment_meta.spans_of("body")
            ...
            if segment_meta is None:
                ...
                if _diag.hit("rag_spans_no_reference"):
                    logger.warning(f"[diag] {path}@{start_sec:.1f}s: has lookup/body spans "
            ...
            if segment_meta is not None and reference_text:
                text_tokens, audio_tokens, segment_meta, injected_lookup, injected_ref = self._inject_rag_blocks(
                    text_tokens=text_tokens,
                    audio_tokens=audio_tokens,
                    segment_meta=segment_meta,
                    reference_text=reference_text,
                    this_num_audio_frames=this_num_audio_frames,
                    path=path,
                    start_sec=start_sec,
                )
